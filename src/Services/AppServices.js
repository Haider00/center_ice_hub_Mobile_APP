...
class AppService extends React.Component {
  storeLoginData = async (
    ...
    try {
      ...
      // await AsyncStorage.setItem('@admob_status', admob_status)
  ...
  storePurchased = async (dmob_status) => {
    try {
      await AsyncStorage.setItem('@admob_status', admob_status);
      return true;
    } catch (e) {
  ...
  getAdmubStatus = async () => {
    try {
      const value = await AsyncStorage.getItem('@admob_status');
      if (value !== null) {
        return value;
      }
      return false;
